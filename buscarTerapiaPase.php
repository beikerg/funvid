      ...
      $busqueda = strtolower($_REQUEST['busqueda']);
        ...
        if(empty($busqueda))
        {
          ?><script type="text/javascript">
            location.href="ListaTerapiaRelatoPase.php";
          </script><?php
          mysqli_close($mysql);
        }
...
<script type="text/javascript" src="js/tera-pase.js"></script>
      ...
      <div class="modal-footer">
        ...
        <div class="col-md-12">
          <div class="box box-success">
            <div class="box-header with-border">
              ...
              <form action="buscarTerapiaPase.php" method="GET">
        ...
        $sql = "SELECT COUNT(*) as total_registro FROM tera_pase t inner join residentes r ON t.id_residente = r.id_residente
                WHERE ( t.id_t_pase LIKE '%$busqueda%' OR 
                       t.nombre_director LIKE '%$busqueda%' OR 
                       t.fecha_t LIKE '%$busqueda%' OR
        ...
        $q = "SELECT r.id_residente, r.nombre, r.apellido, r.rut, t.id_t_pase, t.nombre_director, t.fecha_t FROM tera_pase t inner join residentes r ON t.id_residente = r.id_residente
            WHERE ( t.id_t_pase LIKE '%$busqueda%' OR 
                   t.nombre_director LIKE '%$busqueda%' OR 
                   t.fecha_t LIKE '%$busqueda%' OR
                    r.nombre LIKE '%b%' OR
                   r.apellido LIKE '%$busqueda%' OR
                   r.rut LIKE '%$busqueda%' )
           ORDER BY t.id_t_pase ASC LIMIT $desde,$por_pagina";
        ...
        if($resultado > 0){
          while ($data = mysqli_fetch_array($query)) {
            ...
            <tr>
              <td><?php echo $data['id_t_pase']; ?></td>
              <td><?php echo $data['nombre']; ?></td>
              <td><?php echo $data['apellido']; ?></td>
              <td><?php echo $data['rut']; ?></td>
              <td><?php echo $data['nombre_director']; ?></td>
              <td><?php echo $data['fecha_t']; ?></td>
              <td>
                  <a class="btn btn-warning" title="Editar" href="editarTerapiaRelatoPase.php?id=<?php echo $data['id_t_pase']; ?>"><i class="glyphicon glyphicon-pencil"></i></a>
                  <a class="btn btn-danger" title="Eliminar" href="#"><i class="glyphicon glyphicon-trash"></i></a>
