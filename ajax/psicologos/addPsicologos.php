<?php
		...
		&& isset($_POST['correo']))
	{
		...
		$ejecutar = "SELECT * FROM psicologos WHERE rut = '$rut' OR correo = '$correo' ";
		$resul= $mysql->query($ejecutar);
		$g= mysqli_num_rows($resul);
		...
		if($g == 1){
			...
			echo "<center><p class='alert alert-warning alert-dismissible'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button><strong>¡Error!</strong> El RUT o correo ingresado ya existe. </p></center>";
				...
				$query = "INSERT INTO psicologos(nombre, apellido, rut, telefono, fecha, direccion, localidad, provincia, correo) VALUES('$nombre', '$apellido', '$rut', '$telefono', '$fecha', '$direccion', '$localidad', '$provincia', '$correo')";

		if (!$result = $mysql->query($query)) {
	        exit(mysqli_error());
	    }
	       echo "<center><p class='alert alert-success alert-dismissible'><button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button><strong>¡Bien Hecho!</strong> El Psicolog se ha agregado con éxito.</p></center>";
